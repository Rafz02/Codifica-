...
function calcularNotas($notinha){
    $calcular= array_sum($notinha);
    $quantidade= count($notinha);
    $media= $calcular / $quantidade;
    return $media;
}
foreach($notasAlunos as $i => $notinha){
    $media= calcularNotas($notinha);
    echo "Aluno: " . $i+1 . "Teve a média: $media \n";
    if ($media > 7){
        $aprovados++;
    }else{
        $reprovados++;
...
echo "Total de reprovados (burrinhos) $reprovados";
echo "Total de aprovados (espertinhos) $aprovados";